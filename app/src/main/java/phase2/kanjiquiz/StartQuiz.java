...
public class StartQuiz extends Activity implements OnClickListener , AdListener {
	...
	private String userAnswer, correctAnswer, correctIndex, optionAns, userSelectIndex;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		/*
		 * Table_Name = new ArrayList<String>(); Table_Name.add("Math");
		 * Table_Name.add("Sport"); Table_Name.add("PopCulture");
		 */
	...
	private void init() {
		...
		/*
		 * if(topicName.equalsIgnoreCase("Trivia")){ int pos =
		 * Utility.generateRandomForTableName(); Utility.mSelectTest =
		 * Table_Name.get(pos); }else {
		 */
	...
	public void setQuestionUI() {
		...
		tvScoreCountDown.setText("Correct: " + score + "  Incorrect: " + incorrect);
		tvQuestion.setText(alQuestionContainer.get(random).get("question").toString() + "?");
		...
		correctAnswer = alQuestionContainer.get(random).get("correct_ans").toString();
		try {
			...
			if (Utility.mOptionAorB == 0) {
				...
				optionAns = alQuestionContainer.get(random).get(temp).toString();

				// optionAns =
				// alQuestionContainer.get(random).get("option_b_ans")
				// .toString();
			} else {
				...
				/*
				 * optionAns =
				 * alQuestionContainer.get(random).get("option_a_ans")
				 * .toString();
				 */
			}
		} catch (Exception e) {
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (count < 25) {
			...
			Toast.makeText(StartQuiz.this, "Quiz Over!", Toast.LENGTH_LONG).show();
		}
	...
	public void onClick(View v) {
		...
		if (count < 10) {
			...
			dbHelper.openDataBase();
			if (Utility.mOptionAorB == 0) {
				dbHelper.insertAnswer(Utility.mSelectTest, "option_a_ans", userAnswer,
						String.valueOf(alQuestionContainer.get(random).get("id")));
				if (count == 10) {
					dbHelper.insertAnswer1("HighScore", "option_a_score", String.valueOf(score), Utility.mSelectTest);
					dbHelper.close();
				}
			} else {
				dbHelper.insertAnswer(Utility.mSelectTest, "option_b_ans", userAnswer,
						String.valueOf(alQuestionContainer.get(random).get("id")));
				if (count == 10) {
					dbHelper.insertAnswer1("HighScore", "option_b_score", String.valueOf(score), Utility.mSelectTest);
					dbHelper.close();
				}
			}
			if (count == 10) {
			...
			Toast.makeText(getApplicationContext(), "Test is Over!", Toast.LENGTH_LONG).show();
		}
	...
	public void showWarningDialog() {
		AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(StartQuiz.this);

		alertDialogBuilder.setTitle("Alert!");
		alertDialogBuilder.setMessage("ARE YOU QUITE QUIZ?").setCancelable(false)
				.setPositiveButton("Yes", new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int id) {
						Intent i = new Intent(StartQuiz.this, SplasScreen.class);
						startActivity(i);
						StartQuiz.this.finish();
					}
